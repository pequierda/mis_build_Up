    ...
    return `
        <div class="contact-card bg-white rounded-xl shadow-lg p-6 border border-gray-100 hover:shadow-xl transition-shadow">
            <div class="text-center">
                ...
                <p class="text-blue-600 text-xs font-medium mb-2">${contact.type}</p>
                <div class="flex items-center justify-center">
                    <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${contact.isActive ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}">
...
async function editContact(contactId) {
    try {
        ...
        showNotification('Failed to load contact: ' + error.message, 'error');
    }
}
...
async function handleFormSubmit(e) {
    ...
    const formData = {
        ...
        icon: document.getElementById('contactIcon').value.trim(),
...
function initializeEventListeners() {
    // Modal controls
    document.getElementById('addContactBtn').addEventListener('click', () => openModal('Add New Contact'));
